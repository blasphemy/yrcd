...
static gboolean __lambda2_ (yrcdyrcd_user* self);
...
static guint yrcd_yrcd_user_setup_ping_timer (yrcdyrcd_user* self) {
	...
	_tmp0_ = g_timeout_add_seconds_full (G_PRIORITY_DEFAULT, (guint) 10, ___lambda2__gsource_func, g_object_ref (self), g_object_unref);
	t = _tmp0_;
	result = t;
	return result;
}
...
gchar* yrcd_yrcd_user_get_ip (yrcdyrcd_user* self) {
	...
	g_return_val_if_fail (self != NULL, NULL);
	{
		...
		GInetSocketAddress* _tmp3_ = NULL;
		gchar* ip = NULL;
		...
		_tmp3_ = G_TYPE_CHECK_INSTANCE_TYPE (_tmp0_, g_inet_socket_address_get_type ()) ? ((GInetSocketAddress*) _tmp0_) : NULL;
		if (_tmp3_ == NULL) {
			_g_object_unref0 (_tmp0_);
		}
		inetsockaddr = _tmp3_;
		_tmp4_ = inetsockaddr;
		_tmp5_ = g_inet_socket_address_get_address (_tmp4_);
		_tmp6_ = g_inet_address_to_string (_tmp5_);
		ip = _tmp6_;
		result = ip;
	...
	__catch3_g_error:
	{
		...
		_tmp10_ = g_strdup_printf ("Error getting user ip: %s", _tmp9_);
		_tmp11_ = _tmp10_;
...
void yrcd_yrcd_user_user_reg (yrcdyrcd_user* self, gchar** args, int args_length1) {
	...
	if (!_tmp0_) {
		...
		_tmp26_ = _tmp25_;
		yrcd_yrcd_user_set_realname (self, _tmp26_);
		_g_free0 (_tmp26_);
		yrcd_yrcd_user_set_user_set (self, TRUE);
		_tmp27_ = yrcd_yrcd_user_isnickset (self);
		if (_tmp27_) {
			yrcd_yrcd_user_reg_finished (self);
		}
		...
		gchar* _tmp30_ = NULL;
		gchar* _tmp31_ = NULL;
		_tmp28_ = self->priv->_server;
		_tmp29_ = self->priv->_id;
		_tmp30_ = g_strdup_printf ("User %d attempted user registration while already registered", _tmp29_);
		_tmp31_ = _tmp30_;
		yrcd_yrcd_server_log (_tmp28_, _tmp31_);
		_g_free0 (_tmp31_);
...
static gboolean yrcd_yrcd_user_hostname_lookup_co (YrcdYrcdUserHostnameLookupData* _data_) {
	...
	{
		_data_->_tmp10_ = NULL;
		_data_->_tmp10_ = _data_->resolv;
		_data_->_tmp11_ = NULL;
		_data_->_tmp11_ = _data_->hn;
		_data_->_state_ = 2;
		g_resolver_lookup_by_name_async (_data_->_tmp10_, _data_->_tmp11_, NULL, yrcd_yrcd_user_hostname_lookup_ready, _data_);
		return FALSE;
		_state_2:
		_data_->_tmp12_ = NULL;
		_data_->_tmp12_ = g_resolver_lookup_by_name_finish (_data_->_tmp10_, _data_->_res_, &_data_->_inner_error_);
		_data_->_tmp9_ = _data_->_tmp12_;
		if (_data_->_inner_error_ != NULL) {
			goto __catch8_g_error;
		}
		__g_list_free__g_object_unref0_0 (_data_->addresses);
		_data_->addresses = _data_->_tmp9_;
	}
	goto __finally8;
	__catch8_g_error:
	{
		...
		yrcd_yrcd_user_send_notice (_data_->self, "*** Unable to resolve your hostname");
		_data_->_tmp13_ = NULL;
		_data_->_tmp13_ = _data_->self->ip;
		_data_->_tmp14_ = NULL;
		_data_->_tmp14_ = g_strdup (_data_->_tmp13_);
		_g_free0 (_data_->self->host);
		_data_->self->host = _data_->_tmp14_;
	...
	_data_->_tmp15_ = NULL;
	_data_->_tmp15_ = _data_->addresses;
	{
		_data_->k_collection = _data_->_tmp15_;
		for (_data_->k_it = _data_->k_collection; _data_->k_it != NULL; _data_->k_it = _data_->k_it->next) {
			_data_->_tmp16_ = NULL;
			_data_->_tmp16_ = _g_object_ref0 ((GInetAddress*) _data_->k_it->data);
			_data_->k = _data_->_tmp16_;
			{
				_data_->_tmp17_ = NULL;
				_data_->_tmp17_ = _data_->k;
				_data_->_tmp18_ = NULL;
				_data_->_tmp18_ = g_inet_address_to_string (_data_->_tmp17_);
				_data_->_tmp19_ = NULL;
				_data_->_tmp19_ = _data_->_tmp18_;
				_data_->_tmp20_ = NULL;
				_data_->_tmp20_ = _data_->self->ip;
				_data_->_tmp21_ = FALSE;
				_data_->_tmp21_ = g_strcmp0 (_data_->_tmp19_, _data_->_tmp20_) == 0;
