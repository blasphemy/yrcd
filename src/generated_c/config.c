...
yrcdConfig* yrcd_config_construct (GType object_type, const gchar* filepath) {
	...
	self = (yrcdConfig*) g_object_new (object_type, NULL);
	{
		...
		_tmp9__length1 = tmplist_length1;
		{
			...
			for (k_it = 0; k_it < _tmp9__length1; k_it = k_it + 1) {
				...
				k = k_collection[k_it];
				{
					gint _tmp10_ = 0;
					_tmp10_ = k;
					self->listen_ports = g_list_append (self->listen_ports, (gpointer) ((guintptr) ((guint16) _tmp10_)));
				}
			}
		}
		_tmp12_ = self->priv->file;
		_tmp14_ = g_key_file_get_string_list (_tmp12_, "Networking", "listening_ips", &_tmp13_, &_inner_error_);
		_tmp11_ = _tmp14_;
		...
		_tmp15_ = _tmp11_;
		_tmp15__length1 = _tmp11__length1;
		_tmp11_ = NULL;
		_tmp11__length1 = 0;
		self->listen_ips = (_vala_array_free (self->listen_ips, self->listen_ips_length1, (GDestroyNotify) g_free), NULL);
		self->listen_ips = _tmp15_;
		self->listen_ips_length1 = _tmp15__length1;
		_tmp16_ = self->priv->file;
		_tmp17_ = g_key_file_get_string (_tmp16_, "ServerVariables", "motd_path", &_inner_error_);
		motdpath = _tmp17_;
		if (_inner_error_ != NULL) {
		...
		_tmp23_ = g_data_input_stream_new ((GInputStream*) _tmp20_);
		dis = _tmp23_;
		while (TRUE) {
			gchar* _tmp24_ = NULL;
			...
			gchar* _tmp30_ = NULL;
			_tmp25_ = dis;
			_tmp26_ = g_data_input_stream_read_line (_tmp25_, NULL, NULL, &_inner_error_);
			_tmp24_ = _tmp26_;
			if (_inner_error_ != NULL) {
				_g_free0 (motd_line);
			...
			self->motd = g_list_append (self->motd, _tmp30_);
			_g_free0 (_tmp24_);
		}
		_tmp32_ = self->priv->file;
		_tmp33_ = g_key_file_get_integer (_tmp32_, "ServerVariables", "ping_invertal", &_inner_error_);
		_tmp31_ = _tmp33_;
		if (_inner_error_ != NULL) {
			_g_free0 (motd_line);
			_g_object_unref0 (dis);
		...
		self->ping_invertal = _tmp31_;
		_tmp35_ = self->priv->file;
		_tmp36_ = g_key_file_get_string (_tmp35_, "ServerVariables", "host_salt", &_inner_error_);
		_tmp34_ = _tmp36_;
		if (_inner_error_ != NULL) {
			_g_free0 (motd_line);
			_g_object_unref0 (dis);
