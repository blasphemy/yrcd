...
void yrcd_yrcd_router_join_handler (yrcdyrcd_router* self, yrcdyrcd_user* user, gchar** args, int args_length1) {
	...
	if (_tmp5_) {
		...
		_tmp6_ = yrcd_yrcd_channel_new ();
		newchan = _tmp6_;
		_tmp7_ = newchan;
		_tmp8_ = args;
		_tmp8__length1 = args_length1;
		_tmp9_ = _tmp8_[1];
		yrcd_yrcd_channel_set_name (_tmp7_, _tmp9_);
		_tmp10_ = self->server;
		_tmp11_ = _tmp10_->channellist;
		_tmp12_ = self->server;
		_tmp13_ = yrcd_yrcd_server_new_cid (_tmp12_);
		_tmp14_ = newchan;
		gee_abstract_map_set ((GeeAbstractMap*) _tmp11_, (gpointer) ((gintptr) _tmp13_), _tmp14_);
		_tmp15_ = newchan;
		_tmp16_ = user;
		yrcd_yrcd_channel_add_user (_tmp15_, _tmp16_);
